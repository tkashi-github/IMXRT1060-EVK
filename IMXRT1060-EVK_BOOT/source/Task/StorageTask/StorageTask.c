...
DefALLOCATE_ITCM static _Bool StorageInit(enSD_t enSlotNo, sd_card_t *card)
{
	...
	{
		mimiclib_printf("SD_HOST_CLK_FREQ       = %lu\r\n", SD_HOST_CLK_FREQ);
		mimiclib_printf("card->busClock_Hz      = %lu\r\n", card->busClock_Hz);
		mimiclib_printf("card->version          = %lu\r\n", card->version);
		mimiclib_printf("card->currentTiming    = %lu\r\n", card->currentTiming);
		mimiclib_printf("card->operationVoltage = %lu\r\n", card->operationVoltage);
		mimiclib_printf("card->blockCount       = %lu\r\n", card->blockCount);
		mimiclib_printf("card->blockSize        = %lu\r\n", card->blockSize);
		mimiclib_printf("<Initial Time = %lu msec>\r\n", xTaskGetTickCount() - StartTick);
		bret = true;
	}
	else
	{
		mimiclib_printf("[%s (%d)] SD_CardInit NG <%d> (Slot = %d)!\r\n", __FUNCTION__, __LINE__, sts, enSlotNo + 1);
	}
	return bret;
}
...
DefALLOCATE_ITCM static void StorageDeinit(enSD_t enSlotNo, sd_card_t *card)
{
	...
	mimiclib_printf("[%s (%d)] StorageDeinit OK (Slot = %d)!\r\n", __FUNCTION__, __LINE__, enSlotNo + 1);
}
...
DefALLOCATE_ITCM static void StorageMount(enSD_t enSlotNo)
{
	...
	if (sts == FR_OK)
	{
		mimiclib_printf("f_mount OK (Slot = %d)\r\n", enSlotNo + 1);
	}
	else
	{
		mimiclib_printf("f_mount NG <%d> (Slot = %d)\r\n", sts, enSlotNo + 1);
	}
}
...
DefALLOCATE_ITCM static void StorageUnmount(enSD_t enSlotNo)
{
	...
	if (sts == FR_OK)
	{
		mimiclib_printf("f_unmount OK (Slot = %d)!\r\n", enSlotNo + 1);
	}
	else
	{
		mimiclib_printf("f_unmount NG <%d> (Slot = %d)!\r\n", sts, enSlotNo + 1);
	}
}
...
DefALLOCATE_ITCM static void StorageTaskActual(enSD_t enSlotNo)
{
	...
	if (sizeof(stTaskMsg) <= xStreamBufferReceive(g_sbhStorageTask[enSlotNo], &stTaskMsg, sizeof(stTaskMsg), portMAX_DELAY))
	{
		...
		case enSDRemoved:
			...
			break;
		default:
			mimiclib_printf("[%s (%d)] Unkown Msg (Slot = %d)!\r\n", __FUNCTION__, __LINE__, enSlotNo + 1);
			break;
		}
...
DefALLOCATE_ITCM void StorageTask(void const *argument)
{
	...
	DebugTraceX("DEBUG", dbgMINOR, "Start  (Slot = %d)!", enSlotNo + 1);
	...
	{
		mimiclib_printf("[%s (%d)] SD_HostInit NG (Slot = %d)!\r\n", __FUNCTION__, __LINE__, enSlotNo + 1);
		vTaskSuspend(NULL);
	}
...
void CmdStorageDump(uint32_t argc, const char *argv[])
{
	...
	{
		mimiclib_printf("StorageDump <SectorNo>\r\n");
	}
}
